...
| Alt app | `app.rs` (398 lines) | An earlier `CliApp` prototype with `ConversationClient`, stream event handling, `TerminalRenderer`, output format support | unused/legacy |
...
| 11 | Dual app structs — `app.rs` has a separate `CliApp` that duplicates `LiveCli` from `main.rs` | Navigation confusion; dead code |
...
| 0.2 | Remove or merge the legacy `CliApp` — the existing `app.rs` has an unused `CliApp` with its own `ConversationClient`-based rendering. Delete it or merge its unique features (stream event handler pattern) into the active `LiveCli` | S |
| 0.3 | Consolidate `main.rs` arg parsing — the current `parse_args()` is a hand-rolled parser that duplicates the clap-based `args.rs`. Consolidate on the hand-rolled parser (it is more feature-complete) and move it to `args.rs`, or adopt clap fully | S |
