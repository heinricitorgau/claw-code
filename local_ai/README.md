...
- 如果你用完後想回收 repo 內空間，可以執行 `bash cleanup_local.sh`。
...
- `bash cleanup_local.sh` 不會動到 `~/.ollama` 的全域模型快取，避免誤刪你原本就有的模型。
- `powershell -ExecutionPolicy Bypass -File .\cleanup_local.ps1` 也只會清 repo 內的 bundle。
